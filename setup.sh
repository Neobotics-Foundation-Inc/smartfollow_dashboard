...
#
# A first install leaves neoracer-smartfollow.service installed, stopped, and
...
#   restart   restart the service, taking port 8085 back first
...
SVC=neoracer-smartfollow.service
...
# smartfollow.py. Killing a service-owned PID would only trip its own
...
start_and_check() {
    ...
    echo "Smart Follow Dashboard running: http://$(hostname -I | awk '{print $1}'):$PORT"
}
